...
func Services(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			...
			Response.Data = k8s.ListServices(JsonData.NameSpace)
		}
	}
}
...
func ServiceYaml(c *gin.Context) {
	//ServiceYaml
	var (
		K8sCluster []databases.K8sCluster
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			k8s := common.K8sCluster{Name: K8sCluster[0].K8sName, Config: K8sCluster[0].K8sConfig}
			Response.Data = k8s.GetServicesYaml(JsonData.NameSpace, JsonData.Name)
		}
	}
}
...
func DeleteService(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			k8s := common.K8sCluster{Name: K8sCluster[0].K8sName, Config: K8sCluster[0].K8sConfig}
			err = k8s.DelService(JsonData.NameSpace, JsonData.Name)
		}
	}
}
