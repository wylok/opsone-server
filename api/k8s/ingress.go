...
func Ingress(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			...
			Response.Data = k8s.ListIngress(JsonData.NameSpace)
		}
	}
}
...
// @Router /api/v1/k8s/ingress [delete]
func DeleteIngress(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			k8s := common.K8sCluster{Name: K8sCluster[0].K8sName, Config: K8sCluster[0].K8sConfig}
			err = k8s.DelIngress(JsonData.NameSpace, JsonData.Name)
		}
	}
}
