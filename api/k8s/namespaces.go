...
func NameSpaces(c *gin.Context) {
	...
	var (
		...
		JsonData   k8s_conf.ListNameSpace
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			...
			Response.Data = k8s.ListNameSpaces()
		}
	}
}
...
func AddNamespace(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			k8s := common.K8sCluster{Name: K8sCluster[0].K8sName, Config: K8sCluster[0].K8sConfig}
			err = k8s.AddNameSpace(JsonData.NameSpace)
		}
	}
}
...
// @Param body body  k8s_conf.App true "json数据"
// @Success 200 {} json "{success:true,message:"ok",data:null}"
// @Router /api/v1/k8s/namespace [delete]
func DeleteNamespace(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			k8s := common.K8sCluster{Name: K8sCluster[0].K8sName, Config: K8sCluster[0].K8sConfig}
			err = k8s.DelNameSpace(JsonData.NameSpace)
		}
	}
}
