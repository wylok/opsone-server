...
func Jobs(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			...
			Response.Data = k8s.ListJobs(JsonData.NameSpace)
		}
	}
}
...
// @Produce  json
// @Security ApiKeyAuth
// @Param body body  k8s_conf.Application true "json数据"
// @Success 200 {} json "{success:true,message:"ok",data:null}"
// @Router /api/v1/k8s/job [delete]
func DeleteJob(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			k8s := common.K8sCluster{Name: K8sCluster[0].K8sName, Config: K8sCluster[0].K8sConfig}
			err = k8s.DelJob(JsonData.NameSpace, JsonData.Name)
		}
	}
}
