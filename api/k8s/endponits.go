...
// @Router /api/v1/k8s/endpoints [get]
func Endpoints(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			...
			Response.Data = k8s.ListEndpoints(JsonData.NameSpace)
		}
	}
}
...
// @Router /api/v1/k8s/endpoint [delete]
func DeleteEndpoint(c *gin.Context) {
	...
	if err == nil {
		...
		if len(K8sCluster) > 0 {
			k8s := common.K8sCluster{Name: K8sCluster[0].K8sName, Config: K8sCluster[0].K8sConfig}
			err = k8s.DelEndpoint(JsonData.NameSpace, JsonData.Name)
		}
	}
}
